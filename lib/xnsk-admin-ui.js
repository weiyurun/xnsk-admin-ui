var qe = Object.defineProperty, He = Object.defineProperties;
var Ke = Object.getOwnPropertyDescriptors;
var ze = Object.getOwnPropertySymbols;
var We = Object.prototype.hasOwnProperty, Je = Object.prototype.propertyIsEnumerable;
var Le = (t, l, a) => l in t ? qe(t, l, { enumerable: !0, configurable: !0, writable: !0, value: a }) : t[l] = a, H = (t, l) => {
  for (var a in l || (l = {}))
    We.call(l, a) && Le(t, a, l[a]);
  if (ze)
    for (var a of ze(l))
      Je.call(l, a) && Le(t, a, l[a]);
  return t;
}, me = (t, l) => He(t, Ke(l));
var ke = (t, l, a) => new Promise((e, n) => {
  var v = (w) => {
    try {
      o(a.next(w));
    } catch (r) {
      n(r);
    }
  }, k = (w) => {
    try {
      o(a.throw(w));
    } catch (r) {
      n(r);
    }
  }, o = (w) => w.done ? e(w.value) : Promise.resolve(w.value).then(v, k);
  o((a = a.apply(t, l)).next());
});
import { useAttrs as de, useSlots as Ce, computed as ne, openBlock as d, createBlock as $, unref as s, isRef as Ee, normalizeStyle as he, withCtx as C, createCommentVNode as A, renderSlot as W, createElementBlock as O, createTextVNode as X, toDisplayString as J, createElementVNode as K, createVNode as U, onMounted as De, mergeProps as Te, defineComponent as Fe, ref as V, watchEffect as je, normalizeClass as Ue, Fragment as le, renderList as ce, withKeys as Xe, h as ge, resolveComponent as Ge, resolveDirective as Ye, withDirectives as Ae, vShow as Ze, watch as Qe } from "vue";
import { NModal as et, NButton as ae, NDrawer as tt, NDrawerContent as lt, useDialog as at, NGrid as Ie, NGi as Se, NInput as Be, NSelect as Oe, NIcon as we, NDataTable as nt, NPagination as ot, NForm as st, NFormItem as ut, NRadioGroup as it, NRadio as rt, NCheckboxGroup as ct, NCheckbox as Pe, NSpace as dt, NSpin as ft, NEmpty as pt, NTree as gt, NUpload as ht } from "naive-ui";
import yt from "markdown-it";
import vt from "houriprotocol";
import bt from "axios";
const oe = (t, l) => {
  const a = t.__vccOpts || t;
  for (const [e, n] of l)
    a[e] = n;
  return a;
}, _t = {
  key: 1,
  class: "dialog-templete-btns"
}, kt = {
  __name: "index",
  props: {
    show: {
      type: Boolean,
      default: !1
    },
    title: {
      type: String,
      default: ""
    },
    width: {
      type: [Number, String],
      default: "600"
    },
    height: {
      type: [Number, String],
      default: "auto"
    },
    footBtns: {
      type: Array,
      default: ["确定", "取消"]
    },
    loading: {
      type: Boolean,
      default: !1
    },
    callback: {
      type: Array,
      default: []
    },
    //是否锁定
    lock: {
      type: Boolean,
      defualt: !1
    }
  },
  emits: ["update:show"],
  setup(t, { emit: l }) {
    const a = t, e = de(), n = Ce();
    let v = ne({
      get() {
        return a.show;
      },
      set(w) {
        w || o();
      }
    });
    function k() {
      a.callback[0] && a.callback[0]();
    }
    function o() {
      e.onBeforeClose ? l("beforeClose", () => {
        a.callback[1] && (l("update:show", !1), a.callback[1] && a.callback[1]());
      }) : (l("update:show", !1), a.callback[1] && a.callback[1]());
    }
    return (w, r) => (d(), $(s(et), {
      show: s(v),
      "onUpdate:show": r[0] || (r[0] = (y) => Ee(v) ? v.value = y : v = y),
      "trap-focus": !1,
      preset: "card",
      "close-on-esc": !t.lock,
      "mask-closable": !t.lock,
      class: "dialog-templete",
      style: he({
        width: t.width + "px",
        height: t.height + "px"
      })
    }, {
      header: C(() => [
        t.title ? (d(), $(s($e), {
          key: 0,
          title: t.title
        }, null, 8, ["title"])) : A("", !0)
      ]),
      default: C(() => [
        W(w.$slots, "default", {}, void 0, !0)
      ]),
      footer: C(() => [
        s(n).footBtn ? W(w.$slots, "footBtn", { key: 0 }, void 0, !0) : (d(), O("div", _t, [
          t.footBtns.length > 0 ? (d(), $(s(ae), {
            key: 0,
            type: "info",
            onClick: k
          }, {
            default: C(() => [
              X(J(t.footBtns[0]), 1)
            ]),
            _: 1
          })) : A("", !0),
          t.footBtns.length > 1 ? (d(), $(s(ae), {
            key: 1,
            type: "info",
            ghost: "",
            onClick: o
          }, {
            default: C(() => [
              X(J(t.footBtns[1]), 1)
            ]),
            _: 1
          })) : A("", !0)
        ]))
      ]),
      _: 3
    }, 8, ["show", "close-on-esc", "mask-closable", "style"]));
  }
}, Ve = /* @__PURE__ */ oe(kt, [["__scopeId", "data-v-3d1e23e0"]]), xt = "data:image/png;base64,";
function mt(t) {
  let l = "";
  return Object.prototype.toString.call(t) === "[object String]" ? l = JSON.stringify(t) : l = t, new Promise((a, e) => {
    if (navigator.clipboard)
      navigator.clipboard.writeText(l);
    else {
      let n = document.createElement("textarea");
      document.body.appendChild(n), n.style.position = "fixed", n.style.clip = "rect(0 0 0 0)", n.style.top = "10px", n.value = l, n.select(), document.execCommand("copy", !0), document.body.removeChild(n);
    }
    a();
  });
}
Object.defineProperty(Object.prototype, "xnsk_admin_ui_realType", {
  get() {
    let t = Object.prototype.toString.call(this);
    return t.slice(8, t.length - 1).toLowerCase();
  }
});
Object.defineProperty(Object.prototype, "xnsk_admin_ui_realValue", {
  get() {
    return Object.prototype.toString.call(this) === "[object Function]" ? this() : this;
  }
});
function ye(t = 20, l = !0) {
  let a = "";
  l && (a = (/* @__PURE__ */ new Date()).getTime() + "");
  const e = t - a.length;
  for (let n = 0; n < e; n++)
    a += Me(1);
  return a.substr(0, t);
}
function Me(t = 10) {
  let l = "";
  const a = "1234567890ABCDEFGHIJKLMNOPQRSTUVWXYZ";
  for (let e = 0; e < t; e++)
    l += a.substr(Math.floor(Math.random() * a.length), 1);
  return l;
}
const wt = (t = [], l = "children") => {
  let a = t;
  function e(n) {
    var v;
    ((v = n == null ? void 0 : n[l]) == null ? void 0 : v.length) === 0 ? n[l] = null : n != null && n[l] && n[l].forEach((k) => e(k));
  }
  return (a || []).forEach((n) => e(n)), e(a), a;
}, Nt = (t = [], l = "children") => {
  let a = t;
  function e(n) {
    var v;
    ((v = n == null ? void 0 : n[l]) == null ? void 0 : v.length) === 0 || (n == null ? void 0 : n[l]) === null ? (delete n[l], n.randomId = ye()) : n != null && n[l] && (n.randomId = ye(), n[l].forEach((k) => e(k)));
  }
  return (a || []).forEach((n) => e(n)), e(a), a;
};
function St(t) {
  return new Promise((l, a) => {
    let e;
    window.XMLHttpRequest ? e = new XMLHttpRequest() : e = new ActiveXObject("Microsoft.XMLHttp"), e.onreadystatechange = () => {
      e.readyState == 4 && e.status == 200 && l(e.responseText);
    }, e.onerror = () => {
      a(!1);
    }, e.open("GET", t, !0), e.send();
  });
}
function Bt(t = "", l = "") {
  return l + t.slice(t.lastIndexOf(".") + 1);
}
const Ct = { class: "break-all" }, Tt = { class: "text-center" }, $t = /* @__PURE__ */ K("img", {
  class: "mt-px",
  src: xt,
  style: { width: "16px", height: "16px" }
}, null, -1), At = {
  __name: "index",
  props: {
    show: {
      type: Boolean,
      default: !1
    },
    title: {
      type: String,
      default: "日志"
    },
    data: {
      type: String,
      default: ""
    }
  },
  emits: ["update:show"],
  setup(t, { emit: l }) {
    const a = t;
    Ce();
    let e = ne({
      get() {
        return a.show;
      },
      set(v) {
        v || l("update:show", !1);
      }
    });
    function n() {
      mt(a.data).then(() => {
        try {
          $message.success("复制成功"), l("success");
        } catch (v) {
          console.log("$message未定义"), l("success");
        }
      });
    }
    return (v, k) => (d(), $(s(Ve), {
      title: t.title,
      show: s(e),
      "onUpdate:show": k[0] || (k[0] = (o) => Ee(e) ? e.value = o : e = o),
      width: "800",
      callback: []
    }, {
      footBtn: C(() => [
        K("div", Tt, [
          U(s(ae), {
            type: "primary",
            onClick: n
          }, {
            icon: C(() => [
              $t
            ]),
            default: C(() => [
              X("复制日志 ")
            ]),
            _: 1
          })
        ])
      ]),
      default: C(() => [
        K("div", Ct, J(t.data), 1)
      ]),
      _: 1
    }, 8, ["title", "show"]));
  }
};
const Ot = { class: "flex justify-between items-center" }, jt = { class: "flex justify-start items-center gap-3" }, Ut = { class: "flex justify-center items-center" }, It = {
  __name: "index",
  props: {
    title: String,
    size: {
      type: String,
      default: "small"
    },
    showTitleBar: {
      type: Boolean,
      default: !0
    }
  },
  setup(t) {
    const l = t, a = ne(() => {
      switch (l.size) {
        case "small":
          return "14px";
        case "big":
          return "18px";
        default:
          return l.size;
      }
    });
    return (e, n) => (d(), O("div", Ot, [
      K("div", jt, [
        t.showTitleBar ? (d(), O("div", {
          key: 0,
          class: "blue-bar",
          style: he([{ width: "3px", "border-radius": "2px" }, { height: a.value }])
        }, null, 4)) : A("", !0),
        K("b", {
          style: he({ fontSize: a.value })
        }, J(t.title), 5)
      ]),
      K("div", Ut, [
        W(e.$slots, "default", {}, void 0, !0)
      ]),
      X("   ")
    ]));
  }
}, $e = /* @__PURE__ */ oe(It, [["__scopeId", "data-v-f4694431"]]), zt = {
  __name: "index",
  props: {
    outClosable: {
      type: Boolean,
      default: () => !1
    }
  },
  setup(t) {
    const l = de();
    return De(() => {
    }), (a, e) => (d(), $(s(tt), Te({
      "close-on-esc": t.outClosable,
      "mask-closable": t.outClosable,
      "auto-focus": !1
    }, s(l)), {
      default: C(() => [
        U(s(lt), {
          title: s(l).title,
          closable: ""
        }, {
          default: C(() => [
            W(a.$slots, "default")
          ]),
          _: 3
        }, 8, ["title"])
      ]),
      _: 3
    }, 16, ["close-on-esc", "mask-closable"]));
  }
}, Lt = {
  xmlns: "http://www.w3.org/2000/svg",
  "xmlns:xlink": "http://www.w3.org/1999/xlink",
  viewBox: "0 0 512 512"
}, Rt = /* @__PURE__ */ U(
  "path",
  {
    d: "M221.09 64a157.09 157.09 0 1 0 157.09 157.09A157.1 157.1 0 0 0 221.09 64z",
    fill: "none",
    stroke: "currentColor",
    "stroke-miterlimit": "10",
    "stroke-width": "32"
  },
  null,
  -1
  /* HOISTED */
), Et = /* @__PURE__ */ U(
  "path",
  {
    fill: "none",
    stroke: "currentColor",
    "stroke-linecap": "round",
    "stroke-miterlimit": "10",
    "stroke-width": "32",
    d: "M338.29 338.29L448 448"
  },
  null,
  -1
  /* HOISTED */
), Dt = Fe({
  name: "SearchOutline",
  render: function(l, a) {
    return d(), $("svg", Lt, [Rt, Et]);
  }
}), Ft = {
  xmlns: "http://www.w3.org/2000/svg",
  "xmlns:xlink": "http://www.w3.org/1999/xlink",
  viewBox: "0 0 512 512"
}, Pt = /* @__PURE__ */ U(
  "path",
  {
    d: "M400 148l-21.12-24.57A191.43 191.43 0 0 0 240 64C134 64 48 150 48 256s86 192 192 192a192.09 192.09 0 0 0 181.07-128",
    fill: "none",
    stroke: "currentColor",
    "stroke-linecap": "round",
    "stroke-miterlimit": "10",
    "stroke-width": "32"
  },
  null,
  -1
  /* HOISTED */
), Vt = /* @__PURE__ */ U(
  "path",
  {
    d: "M464 97.42V208a16 16 0 0 1-16 16H337.42c-14.26 0-21.4-17.23-11.32-27.31L436.69 86.1C446.77 76 464 83.16 464 97.42z",
    fill: "currentColor"
  },
  null,
  -1
  /* HOISTED */
), Mt = Fe({
  name: "ReloadOutline",
  render: function(l, a) {
    return d(), $("svg", Ft, [Pt, Vt]);
  }
});
const qt = { class: "list-page-content" }, Ht = {
  key: 0,
  class: "list-page-search marB20"
}, Kt = { class: "flex-1 justify-end flex" }, Wt = {
  key: 1,
  class: "overflow-hidden flex items-center justify-between",
  style: { "margin-bottom": "24px" }
}, Jt = { style: { display: "flex" } }, Xt = { class: "list-page-table" }, Gt = {
  __name: "index",
  props: {
    config: {
      type: [Object, Function],
      default: () => {
      }
    }
  },
  emits: [
    "search",
    "success",
    "finally",
    "rowLoad",
    "rowExpand",
    "onUpdateChecked"
  ],
  setup(t, { expose: l, emit: a }) {
    const e = t, n = V(!1), v = Ce(), k = de(), o = at(), w = ne(() => {
      var u, h, _, j, I;
      return ((_ = (h = (u = e == null ? void 0 : e.config) == null ? void 0 : u.params) == null ? void 0 : h.columns) == null ? void 0 : _.length) > 0 ? (I = (j = e == null ? void 0 : e.config) == null ? void 0 : j.params) == null ? void 0 : I.columns : [];
    }), r = ne(() => {
      var h, _;
      let u = ((_ = (h = e == null ? void 0 : e.config) == null ? void 0 : h.table) == null ? void 0 : _.title) || "";
      return Object.prototype.toString.call(u) === "[object Function]" ? u() : u;
    }), y = V({}), m = V([]);
    let x = V(0);
    const g = V({
      page: 1,
      pageSize: 10,
      pageSizeOptions: [1, 5, 10, 20, 50],
      itemCount: 0,
      isDel: 0
    });
    function S(u) {
      g.value.pageSize = u, f();
    }
    function G(u) {
      g.value.page = u, f();
    }
    function F(u) {
      var h, _, j;
      (j = (_ = (h = e == null ? void 0 : e.config) == null ? void 0 : h.params) == null ? void 0 : _.change) == null || j.call(_, y.value, u);
    }
    let D = V([]), q = V({});
    function Q() {
      w.value.length && w.value.forEach((u) => {
        var h, _;
        u.type === "input" ? y.value[u.propName] = ((h = u == null ? void 0 : u.defaultValue) == null ? void 0 : h.xnsk_admin_ui_realValue) || "" : u.type === "select" && (y.value[u.propName] = u.defaultValue === "" || u.defaultValue === void 0 ? null : (_ = u.defaultValue) == null ? void 0 : _.xnsk_admin_ui_realValue);
      });
    }
    function fe() {
      var h, _, j, I;
      let u = ((_ = (h = e == null ? void 0 : e.config) == null ? void 0 : h.table) == null ? void 0 : _.columns) || [];
      m.value = u.map((z, L) => {
        var Y;
        let M = {
          ellipsis: z.unEllipsis ? !1 : {
            tooltip: !0
          },
          align: (Y = e == null ? void 0 : e.config) != null && Y.isTree && L === 0 ? "left" : z.align || "center",
          titleAlign: "center",
          resizable: !1
          /* ...item, */
        };
        return z.customValue ? M.render = (R) => z.customValue(R) : z.slot ? M.render = (R) => ge(v[z.slot], R) : M.render = (R) => R[z.key] === null || R[z.key] === void 0 || R[z.key] === "" ? "—" : R[z.key], z.key === "actions" && (M.fixed = "right", M.render = (R) => {
          var ie, ve, be;
          let se = [];
          return ((be = (ve = (ie = e == null ? void 0 : e.config) == null ? void 0 : ie.table) == null ? void 0 : ve.actions) == null ? void 0 : be.length) > 0 && e.config.table.actions.forEach((E) => {
            var te, Z, _e;
            let re, pe;
            ((te = E.show) == null ? void 0 : te.xnsk_admin_ui_realType) === "function" ? re = E.show(R) : re = E.show, ((Z = E.disabled) == null ? void 0 : Z.xnsk_admin_ui_realType) === "function" ? pe = E.disabled(R) : pe = (_e = E.disabled) != null ? _e : !1;
            let ue, N = Object.prototype.toString.call(E.icon);
            N === "[object String]" ? ue = ge(
              "img",
              {
                src: E.icon
              },
              { default: () => "" }
            ) : N === "[object Object]" && (ue = E.icon), (re || re === void 0) && se.push(
              ge(
                ae,
                {
                  size: "small",
                  /* loading: _loading, */
                  quaternary: !0,
                  type: E.type || "primary",
                  disabled: pe,
                  onClick: () => {
                    E != null && E.autoWarn ? o.warning({
                      title: `确定${E.label}`,
                      content: "",
                      positiveText: "确定",
                      negativeText: "取消",
                      onPositiveClick: () => {
                        E.click(R);
                      },
                      onNegativeClick: () => {
                      }
                    }) : E.click(R);
                  },
                  style: "--n-opacity-disabled: 0;"
                },
                {
                  default: () => [
                    ue && ge(we, {
                      component: ue
                    }),
                    ge(
                      "div",
                      {
                        style: `margin-left:${ue ? 5 : 0}px;`
                      },
                      {
                        default: () => E.label
                      }
                    )
                  ]
                }
              )
            );
          }), ge(
            "div",
            {
              class: "flex"
            },
            {
              default: () => [...se]
            }
          );
        }), H(H({}, M), z);
      }), (j = e == null ? void 0 : e.config) != null && j.expand && m.value.unshift({
        type: "expand",
        align: "right",
        expandable: (z) => !0,
        renderExpand: (I = e == null ? void 0 : e.config) == null ? void 0 : I.expand
      });
    }
    function p(u = []) {
      let h = Object.prototype.toString.call(u);
      if (h === "[object Array]")
        return u;
      if (h === "[object Object]")
        return Object.keys(u).map((I) => ({
          label: u[I],
          value: I
        }));
    }
    function f(u = {}, h = null) {
      var _, j, I, z;
      if (y.value = H(H({}, y.value), u), ((I = (j = (_ = e == null ? void 0 : e.config) == null ? void 0 : _.data) == null ? void 0 : j.xnsk_admin_ui_realValue) == null ? void 0 : I.length) > 0) {
        if ((z = e == null ? void 0 : e.config) != null && z.isTree) {
          let L = e.config.data.xnsk_admin_ui_realValue.map((M) => me(H({}, M), { randomId: ye() }));
          D.value = Nt(L);
        } else
          D.value = e.config.data.xnsk_admin_ui_realValue;
        g.value.itemCount = e.config.data.xnsk_admin_ui_realValue.length;
        return;
      }
      e.config.apiName && (n.value = !0, e.config.apiName(me(H({}, y.value), {
        pageNum: g.value.page,
        pageSize: g.value.pageSize
      })).then((L) => {
        var M, Y, R;
        if (h && h(L), L.status === 200) {
          let se = (L == null ? void 0 : L.list) || ((M = L == null ? void 0 : L.data) == null ? void 0 : M.list) || [];
          (Y = e == null ? void 0 : e.config) != null && Y.isTree ? D.value = se.map((ie) => me(H({}, ie), { getRandomId: ye() })) : D.value = se, q.value = L || {}, g.value.itemCount = L.total || L.data.total, g.value.page = L.pageNum || L.data.pageNum, a("success", q.value);
        } else
          (R = e == null ? void 0 : e.config) != null && R.showErrMsg && $message.error(L.message);
      }).finally(() => {
        n.value = !1, a("finally");
      }));
    }
    function B(u) {
      return new Promise((h, _) => {
        a("rowLoad", u, (j = []) => {
          u.children = j.map((I) => H({}, I)), h();
        });
      });
    }
    const b = ne(() => {
      var u, h, _, j, I, z, L, M, Y, R;
      return ((u = e == null ? void 0 : e.config) == null ? void 0 : u.expandedIndex) !== void 0 ? (R = (Y = (z = (_ = D.value[(h = e.config) == null ? void 0 : h.expandedIndex]) == null ? void 0 : _.did) != null ? z : (I = D.value[(j = e.config) == null ? void 0 : j.expandedIndex]) == null ? void 0 : I.id) != null ? Y : (M = D.value[(L = e.config) == null ? void 0 : L.expandedIndex]) == null ? void 0 : M.randomId) != null ? R : "" : null;
    });
    function c() {
      g.value.page = 1, f(), a("search");
    }
    function P() {
      return ke(this, null, function* () {
        for (let u in y.value)
          y.value[u] = null;
        c(), a("search");
      });
    }
    function i(u) {
      var h;
      return u != null && u.show ? ((h = u.show) == null ? void 0 : h.xnsk_admin_ui_realType) === "function" ? s(u.show()) : s(u.show) : !0;
    }
    function ee(u) {
      var h;
      return u != null && u.disabled ? ((h = u.disabled) == null ? void 0 : h.xnsk_admin_ui_realType) === "function" ? s(u.disabled()) : s(u.disabled) : !1;
    }
    function T(u) {
      return (u == null ? void 0 : u.xnsk_admin_ui_realType) === "function" ? s(u()) : s(u);
    }
    function xe(u) {
      var h, _;
      ((_ = (h = e.config) == null ? void 0 : h.onUpdateChecked) == null ? void 0 : _.xnsk_admin_ui_realType) === "function" ? e.config.onUpdateChecked(u) : a("onUpdateChecked", u);
    }
    return De(() => {
      var u;
      Q(), fe(), !((u = e.config) != null && u.unLoad) && f(), x.value = m.value.reduce((h, _) => h + (_.minWidth || _.width || 0), 0);
    }), je(() => {
      var u, h, _, j, I;
      ((h = (u = e.config) == null ? void 0 : u.data) == null ? void 0 : h.xnsk_admin_ui_realType) === "asyncfunction" ? (j = (_ = e.config) == null ? void 0 : _.data) == null || j.call(_).then((z) => {
        D.value = z;
      }) : ((I = e.config) == null ? void 0 : I.data) !== void 0 && f();
    }), l({
      getTableData: f,
      refresh: f,
      pageData: q,
      loading: n,
      params: y
    }), (u, h) => {
      var _, j, I, z, L, M, Y, R, se, ie, ve, be, E, re, pe, ue;
      return d(), O("div", {
        class: Ue(["list-page", { noPadding: (_ = e == null ? void 0 : e.config) == null ? void 0 : _.noPadding }]),
        style: he({ height: (I = (j = e == null ? void 0 : e.config) == null ? void 0 : j.height) != null ? I : "100%" })
      }, [
        K("div", qt, [
          W(u.$slots, "top", {
            dataList: s(D),
            loading: n.value
          }, void 0, !0),
          w.value.length > 0 && !t.config.params.hidden ? (d(), O("div", Ht, [
            U(s(Ie), {
              "x-gap": "12",
              cols: "3 s:3 m:4 l:5 xl:6",
              responsive: "screen"
            }, {
              default: C(() => [
                (d(!0), O(le, null, ce(w.value, (N, te) => (d(), O(le, null, [
                  N.hidden ? A("", !0) : (d(), $(s(Se), {
                    key: (N == null ? void 0 : N.did) || te
                  }, {
                    default: C(() => [
                      N.type === "input" ? (d(), $(s(Be), {
                        key: 0,
                        value: y.value[N.propName],
                        "onUpdate:value": [(Z) => y.value[N.propName] = Z, (Z) => F(N)],
                        placeholder: N.placeholder || "请输入" + N.label,
                        clearable: "",
                        disabled: N.disabled === !0 || !1,
                        maxlength: N.maxlength || 20,
                        onKeyup: Xe(c, ["enter"])
                      }, null, 8, ["value", "onUpdate:value", "placeholder", "disabled", "maxlength", "onKeyup"])) : A("", !0),
                      N.type === "select" ? (d(), $(s(Oe), {
                        key: 1,
                        value: y.value[N.propName],
                        "onUpdate:value": [(Z) => y.value[N.propName] = Z, (Z) => F(N)],
                        options: p(N.selection),
                        placeholder: N.placeholder || "请选择" + N.label,
                        clearable: "",
                        disabled: N.disabled === !0 || !1
                      }, null, 8, ["value", "onUpdate:value", "options", "placeholder", "disabled"])) : A("", !0)
                    ]),
                    _: 2
                  }, 1024))
                ], 64))), 256)),
                U(s(Se), { suffix: !0 }, {
                  default: C(() => [
                    K("div", Kt, [
                      U(s(ae), {
                        type: "primary",
                        onClick: c
                      }, {
                        icon: C(() => [
                          U(s(we), null, {
                            default: C(() => [
                              U(s(Dt))
                            ]),
                            _: 1
                          })
                        ]),
                        default: C(() => [
                          X(" 查询")
                        ]),
                        _: 1
                      }),
                      U(s(ae), {
                        class: "ml-3",
                        onClick: P
                      }, {
                        icon: C(() => [
                          U(s(we), null, {
                            default: C(() => [
                              U(s(Mt))
                            ]),
                            _: 1
                          })
                        ]),
                        default: C(() => [
                          X("重置")
                        ]),
                        _: 1
                      })
                    ])
                  ]),
                  _: 1
                })
              ]),
              _: 1
            })
          ])) : A("", !0),
          W(u.$slots, "center", {
            dataList: s(D),
            loading: n.value
          }, void 0, !0),
          (L = (z = e == null ? void 0 : e.config) == null ? void 0 : z.table) != null && L.title || (Y = (M = e == null ? void 0 : e.config) == null ? void 0 : M.table) != null && Y.headBtns ? (d(), O("div", Wt, [
            (se = (R = e == null ? void 0 : e.config) == null ? void 0 : R.table) != null && se.title ? (d(), $(s($e), {
              key: 0,
              size: "16px",
              title: r.value
            }, null, 8, ["title"])) : A("", !0),
            K("p", Jt, [
              (d(!0), O(le, null, ce(((ve = (ie = e == null ? void 0 : e.config) == null ? void 0 : ie.table) == null ? void 0 : ve.headBtns) || [], (N, te) => (d(), O(le, null, [
                i(N) ? (d(), $(s(ae), {
                  type: N.type || "primary",
                  class: "marL15",
                  key: te,
                  loading: T(N.loading),
                  disabled: ee(N),
                  onClick: N.click
                }, {
                  default: C(() => [
                    N.icon ? (d(), $(s(we), {
                      key: 0,
                      component: N.icon,
                      style: { "margin-right": "8px" }
                    }, null, 8, ["component"])) : A("", !0),
                    K("span", null, J(N.label), 1)
                  ]),
                  _: 2
                }, 1032, ["type", "loading", "disabled", "onClick"])) : A("", !0)
              ], 64))), 256))
            ])
          ])) : A("", !0),
          W(u.$slots, "tableHeader", {
            dataList: s(D),
            loading: n.value
          }, void 0, !0),
          K("div", Xt, [
            U(s(nt), Te({
              columns: m.value,
              data: s(D),
              "scroll-x": s(x),
              striped: "",
              loading: n.value,
              "row-key": (N) => {
                var te, Z, _e;
                return (_e = (Z = (te = N.did) != null ? te : N.id) != null ? Z : N.randomId) != null ? _e : "";
              },
              pagination: ((be = e == null ? void 0 : e.config) == null ? void 0 : be.data) && !((E = e == null ? void 0 : e.config) != null && E.unPagination) && {
                page: g.value.page,
                pageSize: g.value.pageSize,
                showSizePicker: !0,
                pageSizes: g.value.pageSizeOptions,
                itemCount: g.value.itemCount,
                showQuickJumper: !0
              },
              "on-update:page-size": S,
              "on-update:page": G,
              "on-update:checked-row-keys": xe,
              "children-key": (re = e == null ? void 0 : e.config) != null && re.isTree ? ((pe = e == null ? void 0 : e.config) == null ? void 0 : pe.childrenKey) || "children" : null,
              "default-expanded-row-keys": [b.value],
              onLoad: B
            }, s(k)), null, 16, ["columns", "data", "scroll-x", "loading", "row-key", "pagination", "children-key", "default-expanded-row-keys"])
          ]),
          W(u.$slots, "footer", {
            dataList: s(D),
            loading: n.value
          }, void 0, !0)
        ]),
        (ue = e == null ? void 0 : e.config) != null && ue.data ? A("", !0) : (d(), $(s(ot), {
          key: 0,
          class: "list-page-table-pagination",
          page: g.value.page,
          "onUpdate:page": h[0] || (h[0] = (N) => g.value.page = N),
          "page-size": g.value.pageSize,
          "onUpdate:pageSize": h[1] || (h[1] = (N) => g.value.pageSize = N),
          "page-sizes": g.value.pageSizeOptions,
          "item-count": g.value.itemCount,
          "on-update:page-size": S,
          "on-update:page": G,
          "show-size-picker": "",
          "show-quick-jumper": ""
        }, null, 8, ["page", "page-size", "page-sizes", "item-count"]))
      ], 6);
    };
  }
}, Yt = /* @__PURE__ */ oe(Gt, [["__scopeId", "data-v-3b6ff612"]]);
const Zt = {
  key: 0,
  class: "form-btns"
}, Qt = {
  __name: "index",
  props: {
    config: {
      type: Object,
      default: {}
    },
    defaultValue: {
      type: Object,
      default: {}
    }
  },
  emits: ["submit", "cancel", "change"],
  setup(t, { expose: l, emit: a }) {
    const e = t, n = {
      "input-select": "请输入",
      input: "请输入",
      textarea: "请输入",
      select: "请选择",
      radio: "请选择",
      checkbox: "请选择",
      slot: "缺少"
    }, v = V("from_" + ye()), k = Ce();
    de();
    const o = V({}), w = V({}), r = ne(() => {
      var p, f, B;
      return (f = (p = e.config) == null ? void 0 : p.submitBtn) != null && f.loading ? ((B = e.config.submitBtn.loading) == null ? void 0 : B.xnsk_admin_ui_realType) === "function" ? s(e.config.submitBtn.loading()) : s(e.config.submitBtn.loading) : !1;
    }), y = ne(() => {
      var f, B;
      let p = [];
      if (((B = (f = e.config) == null ? void 0 : f.columns) == null ? void 0 : B.length) > 0)
        return e.config.columns.forEach((b) => {
          var ee, T, xe, u;
          let c = {};
          c.label = b.label, c.type = b.type, c.useFormItem = b.useFormItem || !1, c.propName = b.propName, c.required = b.required, c.maxlength = b.maxlength, c.rows = b.rows || 9, c.validator = b.validator || null, c.trigger = b.trigger || ((ee = e.config) == null ? void 0 : ee.trigger) || "blur", c.selection = b.selection || [], c.span = b.span || 24, c.offset = b.offset || 0, c.onInput = b.onInput || null, c.text = b.text || "", c.style = b.style || "";
          let P = {
            required: c == null ? void 0 : c.required,
            trigger: c.trigger,
            key: c == null ? void 0 : c.propName,
            validator(h, _) {
              var j, I;
              if (((j = c == null ? void 0 : c.validator) == null ? void 0 : j.xnsk_admin_ui_realType) === "function")
                return c == null ? void 0 : c.validator(_);
              if (((I = c == null ? void 0 : c.validator) == null ? void 0 : I.xnsk_admin_ui_realType) === void 0)
                return c != null && c.required && [null, void 0, ""].includes(_) ? new Error(`${n[c.type]}${c.label}`) : (_ == null ? void 0 : _.xnsk_admin_ui_realType) === "array" && _.length === 0 ? new Error(`${n[c.type]}${c.label}`) : !0;
            }
          };
          w.value[c.propName] = P, b.disabled !== void 0 && (((T = b.disabled) == null ? void 0 : T.xnsk_admin_ui_realType) === "function" ? b.disabled() : b.disabled) && (c.disabled = !0), b.readonly !== void 0 && (((xe = b.readonly) == null ? void 0 : xe.xnsk_admin_ui_realType) === "function" ? b.readonly() : b.readonly) && (c.readonly = !0), (b.show === void 0 || (((u = b.show) == null ? void 0 : u.xnsk_admin_ui_realType) === "function" ? b.show(o.value) : b.show)) && p.push(c);
        }), p;
    });
    m(), je(() => {
      Object.keys(e.defaultValue).forEach((f) => {
        var b;
        let B = (b = e.config) == null ? void 0 : b.columns.find((c) => c.propName === f);
        B && (o.value[f] = S(B));
      });
    });
    function m() {
      var f;
      (((f = e.config) == null ? void 0 : f.columns) || []).forEach((B) => {
        switch (B.type) {
          case "input":
          case "textarea":
            B.propName && (o.value[B.propName] = "");
            break;
          case "select":
            B.propName && (o.value[B.propName] = null);
            break;
        }
      });
    }
    function x(p) {
      return p.placeholder ? p.placeholder : p.type === "select" ? `请选择${p.label}` : `请输入${p.label}`;
    }
    function g(p) {
      var B;
      let f = ((B = p.selection) == null ? void 0 : B.xnsk_admin_ui_realType) === "function" ? s(p.selection()) : s(p.selection);
      return f.xnsk_admin_ui_realType === "array" ? f : Object.keys(f).map((c) => ({
        label: f[c],
        value: c
      }));
    }
    function S(p) {
      var f, B;
      return (B = (f = e.defaultValue) == null ? void 0 : f[p.propName]) != null ? B : p.type === "select" ? null : "";
    }
    function G() {
      var p, f;
      (f = (p = e == null ? void 0 : e.config) == null ? void 0 : p.cancelBtn) != null && f.click ? e.config.cancelBtn.click() : a("cancel");
    }
    function F(p, f) {
      var B, b, c, P;
      if (((B = f == null ? void 0 : f.onInput) == null ? void 0 : B.xnsk_admin_ui_realType) === "function") {
        let i = (b = f == null ? void 0 : f.onInput) == null ? void 0 : b.call(f, p);
        i !== void 0 && (o.value[f.propName] = i);
      }
      (c = e.config) != null && c.change ? (P = e.config) == null || P.change(s(o.value), f, p) : a("change", s(o.value), f, p);
    }
    function D() {
      var p;
      (p = v.value) == null || p.validate((f) => {
        var B, b;
        f || ((b = (B = e.config) == null ? void 0 : B.submitBtn) != null && b.click ? $dialog.warning({
          title: "确定提交？",
          content: "",
          positiveText: "确定",
          negativeText: "取消",
          onPositiveClick: () => {
            var c, P;
            (P = (c = e.config) == null ? void 0 : c.submitBtn) == null || P.click(s(o.value));
          },
          onNegativeClick: () => {
          }
        }) : $dialog.warning({
          title: "确定提交？",
          content: "",
          positiveText: "确定",
          negativeText: "取消",
          onPositiveClick: () => {
            a("submit", s(o.value));
          },
          onNegativeClick: () => {
          }
        }));
      });
    }
    function q(p, f) {
      var B, b;
      if ((f == null ? void 0 : f.xnsk_admin_ui_realType) === void 0)
        (B = v.value) == null || B.validate((c) => p && p(c));
      else {
        let c = f.xnsk_admin_ui_realType === "string" ? [f] : f;
        (b = v.value) == null || b.validate(
          (P) => {
            p && p(P);
          },
          (P) => c.includes(P == null ? void 0 : P.key)
        );
      }
    }
    function Q() {
      return o.value;
    }
    function fe(p = {}) {
      Object.keys(p).forEach((B) => {
        o.value[B] = p[B];
      });
    }
    return l({
      validate: q,
      getValue: Q,
      setValue: fe
    }), (p, f) => {
      var c, P;
      const B = Ge("n-space"), b = Ye("trim");
      return d(), O("div", null, [
        U(s(st), {
          model: o.value,
          rules: w.value,
          ref: (i) => {
            v.value = i;
          },
          "label-placement": "left",
          "label-width": t.config.labelWidth || "auto",
          "require-mark-placement": "right"
        }, {
          default: C(() => [
            U(s(Ie), {
              "x-gap": "12",
              cols: 24
            }, {
              default: C(() => [
                (d(!0), O(le, null, ce(y.value, (i) => (d(), $(s(Se), {
                  class: Ue({ "is-slot": i.type === "slot" && !i.useFormItem }),
                  key: i.propName,
                  span: i.span,
                  offset: i.offset
                }, {
                  default: C(() => [
                    U(s(ut), {
                      label: i.type === "slot" && !i.useFormItem ? null : i.label,
                      path: i.propName
                    }, {
                      default: C(() => {
                        var ee;
                        return [
                          i.type === "input" ? Ae((d(), $(s(Be), {
                            key: 0,
                            value: o.value[i.propName],
                            "onUpdate:value": [(T) => o.value[i.propName] = T, (T) => F(T, i)],
                            placeholder: x(i),
                            maxlength: i.maxlength || 20,
                            disabled: i.disabled,
                            readonly: i.readonly
                          }, null, 8, ["value", "onUpdate:value", "placeholder", "maxlength", "disabled", "readonly"])), [
                            [b]
                          ]) : A("", !0),
                          i.type === "input-select" ? (d(), $(s(Oe), {
                            key: 1,
                            class: "input-select-select",
                            value: o.value[i.propName],
                            options: g(i),
                            placeholder: x(i),
                            disabled: i.disabled,
                            readonly: i.readonly,
                            "show-on-focus": "",
                            filterable: "",
                            tag: "",
                            "ignore-composition": !1,
                            onInput: (T) => {
                              o.value[i.propName] = T.target.value;
                            },
                            "onUpdate:value": (T) => {
                              o.value[i.propName] = T;
                            }
                          }, null, 8, ["value", "options", "placeholder", "disabled", "readonly", "onInput", "onUpdate:value"])) : A("", !0),
                          i.type === "textarea" ? Ae((d(), $(s(Be), {
                            key: 2,
                            value: o.value[i.propName],
                            "onUpdate:value": [(T) => o.value[i.propName] = T, (T) => F(T, i)],
                            type: "textarea",
                            placeholder: x(i),
                            maxlength: i.maxlength || 200,
                            rows: i.rows || 9,
                            "show-count": "",
                            disabled: i.disabled,
                            readonly: i.readonly
                          }, null, 8, ["value", "onUpdate:value", "placeholder", "maxlength", "rows", "disabled", "readonly"])), [
                            [b]
                          ]) : A("", !0),
                          i.type === "select" ? (d(), $(s(Oe), {
                            key: 3,
                            value: o.value[i.propName],
                            "onUpdate:value": [(T) => o.value[i.propName] = T, (T) => F(T, i)],
                            options: g(i),
                            placeholder: x(i),
                            disabled: i.disabled,
                            readonly: i.readonly
                          }, null, 8, ["value", "onUpdate:value", "options", "placeholder", "disabled", "readonly"])) : A("", !0),
                          i.type === "radio" ? (d(), $(s(it), {
                            key: 4,
                            value: o.value[i.propName],
                            "onUpdate:value": [(T) => o.value[i.propName] = T, (T) => F(T, i)],
                            name: i.propName,
                            disabled: i.disabled,
                            readonly: i.readonly
                          }, {
                            default: C(() => [
                              U(B, null, {
                                default: C(() => [
                                  (d(!0), O(le, null, ce(g(i), (T) => (d(), $(s(rt), {
                                    key: T.value,
                                    value: T.value
                                  }, {
                                    default: C(() => [
                                      X(J(T.label), 1)
                                    ]),
                                    _: 2
                                  }, 1032, ["value"]))), 128))
                                ]),
                                _: 2
                              }, 1024)
                            ]),
                            _: 2
                          }, 1032, ["value", "onUpdate:value", "name", "disabled", "readonly"])) : A("", !0),
                          i.type === "checkbox" ? (d(), $(s(ct), {
                            key: 5,
                            value: o.value[i.propName],
                            "onUpdate:value": (T) => o.value[i.propName] = T
                          }, {
                            default: C(() => [
                              X(J(o.value[i.propName]) + " ", 1),
                              U(B, { "item-style": "display: flex;" }, {
                                default: C(() => [
                                  (d(!0), O(le, null, ce(g(i), (T) => (d(), $(s(Pe), {
                                    key: T.value,
                                    value: T.value,
                                    label: T.label
                                  }, null, 8, ["value", "label"]))), 128))
                                ]),
                                _: 2
                              }, 1024)
                            ]),
                            _: 2
                          }, 1032, ["value", "onUpdate:value"])) : A("", !0),
                          i.type === "text" ? (d(), O("span", {
                            key: 6,
                            style: he(i.style)
                          }, J(((ee = i.text) == null ? void 0 : ee.xnsk_admin_ui_realType) === "function" ? i.text() : i.text), 5)) : A("", !0),
                          i.type === "slot" ? W(p.$slots, i.propName, {
                            key: 7,
                            data: o.value,
                            item: i
                          }, void 0, !0) : A("", !0)
                        ];
                      }),
                      _: 2
                    }, 1032, ["label", "path"])
                  ]),
                  _: 2
                }, 1032, ["class", "span", "offset"]))), 128))
              ]),
              _: 3
            })
          ]),
          _: 3
        }, 8, ["model", "rules", "label-width"]),
        s(k).btns ? A("", !0) : (d(), O("div", Zt, [
          (c = t.config) != null && c.submitBtn ? (d(), $(s(ae), {
            key: 0,
            class: "btn",
            type: "primary",
            loading: r.value,
            onClick: D
          }, {
            default: C(() => {
              var i, ee;
              return [
                X(J(((ee = (i = t.config) == null ? void 0 : i.submitBtn) == null ? void 0 : ee.label) || "确定"), 1)
              ];
            }),
            _: 1
          }, 8, ["loading"])) : A("", !0),
          (P = t.config) != null && P.submitBtn ? (d(), $(s(ae), {
            key: 1,
            onClick: G
          }, {
            default: C(() => [
              X(" 取消 ")
            ]),
            _: 1
          })) : A("", !0)
        ])),
        s(k).btns ? W(p.$slots, "btns", { key: 1 }, void 0, !0) : A("", !0)
      ]);
    };
  }
}, el = /* @__PURE__ */ oe(Qt, [["__scopeId", "data-v-c14b9bcf"]]);
const tl = {
  class: "form-btns flex justify-center sticky marB20 relative",
  style: { bottom: "0", padding: "20px", "background-color": "#fff", "z-index": "103" }
}, ll = {
  __name: "index",
  setup(t) {
    return (l, a) => (d(), O("div", tl, [
      U(s(dt), { size: [40, 20] }, {
        default: C(() => [
          W(l.$slots, "default", {}, void 0, !0)
        ]),
        _: 3
      })
    ]));
  }
}, al = /* @__PURE__ */ oe(ll, [["__scopeId", "data-v-d8c9c3a7"]]);
const nl = {
  class: "info-page-value inline-block marL5 padR10 break-all",
  else: ""
}, ol = {
  __name: "index",
  props: {
    config: {
      type: Object,
      defualt: () => {
      }
    },
    data: {
      type: Object,
      defualt: () => {
      }
    }
  },
  setup(t) {
    const l = t;
    de();
    function a(e) {
      var n;
      return typeof e == "string" ? l.data ? (l == null ? void 0 : l.data[e]) || "-" : ((n = l.config) == null ? void 0 : n.data[e]) || "-" : Object.prototype.toString.call(e) === "[object Function]" ? e() : "";
    }
    return (e, n) => {
      var v;
      return d(), O("div", null, [
        t.config.title ? (d(), $(s($e), {
          key: 0,
          class: "marT5 marB15",
          title: t.config.title || ""
        }, null, 8, ["title"])) : A("", !0),
        U(s(Ie), {
          class: Ue({ "has-border": (v = t.config) == null ? void 0 : v.border })
        }, {
          default: C(() => {
            var k;
            return [
              (d(!0), O(le, null, ce(((k = t.config) == null ? void 0 : k.column) || [], (o, w) => {
                var r;
                return d(), $(s(Se), {
                  class: "info-page-item flex items-baseline",
                  key: (o == null ? void 0 : o.did) || w,
                  span: (r = o.span) != null ? r : 8
                }, {
                  default: C(() => {
                    var y, m;
                    return [
                      o.labelSlot ? W(e.$slots, o.labelSlot, { key: 0 }, void 0, !0) : A("", !0),
                      K("span", {
                        class: "info-page-label inline-block padT10 padB10 padR10",
                        else: "",
                        style: he({
                          minWidth: t.config.labelWidth ? t.config.labelWidth + "px" : "auto",
                          "text-align": (y = t.config.labelAlign) != null ? y : "left"
                        })
                      }, J(o.label) + J((m = t.config) != null && m.useColon ? ":" : ""), 5),
                      o.slot ? W(e.$slots, o.slot, { key: 1 }, void 0, !0) : A("", !0),
                      K("span", nl, J(a(o.value)), 1)
                    ];
                  }),
                  _: 2
                }, 1032, ["span"]);
              }), 128))
            ];
          }),
          _: 3
        }, 8, ["class"])
      ]);
    };
  }
}, sl = /* @__PURE__ */ oe(ol, [["__scopeId", "data-v-de364537"]]);
const ul = { class: "markdown-view overflow-hidden html-default-css" }, il = ["innerHTML"], rl = {
  name: "XnskMdPreview"
}, cl = /* @__PURE__ */ Object.assign(rl, {
  props: {
    value: {
      type: String,
      default: ""
    },
    type: {
      type: String,
      default: "url"
    }
  },
  setup(t) {
    const l = t, a = new yt();
    let e = V(!0), n = V("");
    function v() {
      return ke(this, null, function* () {
        e.value = !1, n.value = "";
        let k = yield St(l.value);
        k && (n.value = a.render(k)), e.value = !0;
      });
    }
    return je(() => {
      l.value ? l.type === "url" ? v() : n.value = a.render(l.value) : n.value = "";
    }), (k, o) => (d(), O("div", ul, [
      Ae(U(s(ft), {
        class: "loading",
        size: "large"
      }, null, 512), [
        [Ze, !s(e)]
      ]),
      t.value ? (d(), O("div", {
        key: 0,
        innerHTML: s(n),
        style: { padding: "20px", "border-radius": "6px" }
      }, null, 8, il)) : (d(), $(s(pt), {
        key: 1,
        description: "无数据",
        style: { "margin-top": "60px" }
      }))
    ]));
  }
}), dl = /* @__PURE__ */ oe(cl, [["__scopeId", "data-v-cd3c3a52"]]), fl = ["onClick"], pl = {
  __name: "index",
  props: {
    checked: {
      type: Boolean,
      default: !1
    }
  },
  emits: ["update:checked"],
  setup(t, { emit: l }) {
    const a = V([]);
    function e() {
      vt.init({
        projectId: "20230704143829127",
        sceneCode: "scdl",
        debug: !0,
        authContext: "https://xnsk.mapfarm.com",
        userId: ""
      }).then((k) => {
        var o;
        a.value = (o = k == null ? void 0 : k.agreementList) != null ? o : [];
      }).catch((k) => {
        console.log(k.message || k.msg || "系统异常"), $message.error(k.message || k.msg || "系统异常");
      });
    }
    e();
    function n(k) {
      k && window.open(k);
    }
    function v(k) {
      l("update:checked", k);
    }
    return (k, o) => (d(), O("div", null, [
      U(s(Pe), {
        "default-checked": t.checked,
        "onUpdate:checked": v
      }, null, 8, ["default-checked"]),
      X(" 请阅读 "),
      (d(!0), O(le, null, ce(a.value, (w, r) => (d(), O("span", {
        class: "cursor-pointer",
        style: { color: "#2d8cf0" },
        key: w.did || r,
        onClick: (y) => n(w.agreementUrl)
      }, " 《" + J(w.agreementName) + "》 ", 9, fl))), 128)),
      X("，并同意 ")
    ]));
  }
};
const gl = {
  __name: "index",
  props: {
    value: {
      type: Array,
      default: []
    },
    treeData: {
      type: Array,
      default: []
    },
    dataKey: {
      type: String,
      default: "did"
    },
    label: {
      type: String,
      default: "name"
    },
    childrenKey: {
      type: String,
      default: "children"
    },
    checkable: {
      type: Boolean,
      default: !1
    },
    search: {
      type: Boolean,
      default: !1
    },
    cancelable: {
      type: Boolean,
      default: !1
    }
  },
  emits: ["click", "update:value", "select"],
  setup(t, { emit: l }) {
    const a = t, e = V("tree_" + ye()), n = de(), v = V(""), k = ne(
      () => wt(a.treeData, a.childrenKey || "children")
    );
    function o(x, g, S) {
      l("update:value", r());
    }
    function w(x, g, S) {
      l("update:value", r());
    }
    function r() {
      var F, D, q;
      let x = (D = (F = e.value.getCheckedData()) == null ? void 0 : F.keys) != null ? D : [], g = (q = e.value.getIndeterminateData().keys) != null ? q : [], S = x.map((Q) => [Q, 1]), G = g.map((Q) => [Q, 0]);
      return [...S, ...G];
    }
    function y(x, g, S) {
      l("select", x, g, S);
    }
    function m({ option: x }) {
      return {
        onClick() {
          l("click", x);
        }
      };
    }
    return (x, g) => (d(), O("div", null, [
      t.search ? (d(), $(s(Be), {
        key: 0,
        class: "marB10",
        value: v.value,
        "onUpdate:value": g[0] || (g[0] = (S) => v.value = S),
        placeholder: "搜索",
        clearable: ""
      }, null, 8, ["value"])) : A("", !0),
      U(s(gt), Te({
        ref: (S) => {
          e.value = S;
        },
        "block-line": "",
        data: k.value,
        "default-checked-keys": (t.value || []).filter((S) => S[1] === 1).map((S) => S[0]),
        "indeterminate-keys": (t.value || []).filter((S) => S[1] === 0).map((S) => S[0]),
        "key-field": t.dataKey,
        "label-field": t.label,
        "children-field": t.childrenKey,
        pattern: v.value,
        checkable: t.checkable,
        "node-props": m,
        cancelable: t.cancelable,
        cascade: "",
        selectable: "",
        "show-irrelevant-nodes": !1
      }, s(n), {
        "onUpdate:checkedKeys": o,
        "onUpdate:indeterminateKeys": w,
        "onUpdate:selectedKeys": y
      }), null, 16, ["data", "default-checked-keys", "indeterminate-keys", "key-field", "label-field", "children-field", "pattern", "checkable", "cancelable"])
    ]));
  }
}, hl = /* @__PURE__ */ oe(gl, [["__scopeId", "data-v-7cebb098"]]), Re = (t, l, a = "", e = {}, n = {}) => (t.interceptors.request.use((v) => (l.beforeRequest && l.beforeRequest(v, n != null ? n : {}), v)), new Promise((v, k) => {
  t.post(a, e).then((o) => {
    var w, r;
    ((w = o.data) == null ? void 0 : w.status) !== 200 && ((r = l == null ? void 0 : l.onError) == null || r.call(l, o.data, n != null ? n : {})), v(o.data);
  }).catch((o) => {
    var w, r, y, m;
    (r = l == null ? void 0 : l.onNetWorkError) == null || r.call(
      l,
      (w = o == null ? void 0 : o.response) != null ? w : {
        data: {
          status: 500,
          data: null,
          message: (o == null ? void 0 : o.message) || "系统异常"
        }
      },
      n != null ? n : {}
    ), v(
      (m = (y = o == null ? void 0 : o.response) == null ? void 0 : y.data) != null ? m : {
        status: 500,
        data: null,
        message: o.message
      }
    );
  }).finally(() => {
  });
}));
function yl(t = null) {
  let l = null;
  const a = {
    beforeRequest: null,
    onNetWorkError: null,
    onError: null
  }, e = (r, y) => {
    r.config.globalProperties.$miniHttp = n(y);
  }, n = (r = {}) => {
    a.beforeRequest = (r == null ? void 0 : r.beforeRequest) || null, a.onNetWorkError = (r == null ? void 0 : r.onNetWorkError) || null, a.onError = (r == null ? void 0 : r.onError) || null;
    const y = {
      baseURL: (r == null ? void 0 : r.baseURL) || "",
      timeout: (r == null ? void 0 : r.timeout) || 6e4,
      headers: (() => {
        let m = {}, x = Object.prototype.toString.call(r == null ? void 0 : r.headers);
        return x === "[object Object]" ? m = r.headers : x === "[object Function]" && (m = r.headers()), m;
      })()
    };
    return l = bt.create(y), v.bind(this);
  }, v = (r = {}, y = null) => {
    let m = Object.prototype.toString.call(r);
    if (m === "[object Object]")
      return k(r);
    if (m === "[object String]")
      return r.includes("/") ? o(r, y) : w(r, y);
  }, k = (r = {}) => {
    let y = {};
    return Object.keys(r).forEach((m) => {
      var g, S;
      let x = Object.prototype.toString.call(r[m]);
      x === "[object String]" ? r[m].includes("/") ? y[m] = o(r[m]) : y[m] = w(r[m]) : x === "[object Array]" && (r[m][0].includes("/") ? y[m] = o(r[m][0], (g = r[m]) == null ? void 0 : g[1]) : y[m] = w(r[m][0], (S = r[m]) == null ? void 0 : S[1]));
    }), y;
  }, o = (r = "", y) => (m, x = {}) => Re(l, a, r, m, H(H({}, y || {}), x || {})), w = (r = "", y = {}) => (m = {}, x = {}) => Re(
    l,
    a,
    "/less/api/auth/common",
    {
      apiCode: r,
      queryParam: m
    },
    H(H({}, y || {}), x || {})
  );
  return t ? n(t) : { install: e, create: n };
}
const vl = {
  baseURL: "",
  beforeRequest: (t, l) => {
    var n;
    console.log("[ extendsConfig ] >", l);
    const { token: a = "" } = l;
    t.headers.Authorization = a, t.headers.appcode = "xnsk_admin_ui";
    const e = (n = l == null ? void 0 : l["Content-Type"]) != null ? n : "application/json;charset=UTF-8";
    t.headers["Content-Type"] = e;
  },
  /* 网络异常 */
  onNetWorkError: (t, l) => {
    console.log(" 异常 >", t, l);
  },
  /* 业务异常  */
  onError: (t, l) => {
    console.log(" 异常 >", t, l);
  }
}, bl = yl(vl);
const _l = { style: { width: "100%" } }, kl = { class: "inline-block uploadBtn" }, xl = {
  key: 0,
  style: { "font-size": "40px", "line-height": "20px" }
}, ml = {
  __name: "index",
  props: {
    value: {
      type: String,
      default: ""
    },
    /* 是否显示文件列表 */
    showFileList: {
      type: Boolean,
      default: () => !0
    },
    /* 选中文件是否直接上传 */
    autoUpload: {
      type: Boolean,
      default: () => !0
    },
    // 是否上传多个
    multipleUpload: {
      type: Boolean,
      default: () => !1
    },
    // obs 桶
    bucket: {
      type: String,
      default: "xnsk-base"
    },
    // 存储路径
    path: {
      type: String,
      default: "base/permission"
    },
    url: {
      type: String,
      default: ""
    },
    params: {
      type: Object,
      default: {}
    },
    token: {
      type: String,
      default: ""
    }
  },
  emits: [
    "change",
    "fileChange",
    "update:value",
    "update:fileList"
  ],
  setup(t, { expose: l, emit: a }) {
    const e = t, n = de(), v = V(
      "ref" + (/* @__PURE__ */ new Date()).getTime() + parseInt(Math.random() * 1e4)
    );
    let k = V(!1);
    const o = V([]);
    Qe(
      () => e.value,
      (x) => {
        if ([null, void 0].includes(x)) {
          o.value.splice(0);
          return;
        }
        x._split(",").forEach((S) => {
          o.value.find((F) => F.url === S) || o.value.push({
            url: S,
            status: "finished"
          });
        });
      },
      { deep: !0, immediate: !0 }
    );
    const w = (G) => ke(this, [G], function* ({ file: x, onFinish: g, onError: S }) {
      var Q, fe;
      let F = new FormData();
      F.append("file", x.file), F.append("bucket", e.bucket), F.append("pathPrefix", e.path), Object.keys(e.params).forEach((p) => {
        F.append(p, e.params[p]);
      });
      let q = yield bl(e.url, {
        "Content-Type": "multipart/form-data;charset=UTF-8",
        token: e.token
      })(F);
      if ((q == null ? void 0 : q.status) === 200) {
        const p = Me(10);
        x.url = (fe = (Q = q == null ? void 0 : q.data) == null ? void 0 : Q.url) != null ? fe : "", x.name = p, g();
      } else
        S();
    });
    function r(x) {
    }
    function y(x) {
      return ke(this, null, function* () {
        var g;
        return ((g = n == null ? void 0 : n.accept) == null ? void 0 : g.indexOf(Bt(x.file.name, "."))) === -1 ? ($message.error("格式不正确"), !1) : x.file.size / 1024 > e.maxSize ? ($message.error("文件大小超出限制"), !1) : !0;
      });
    }
    function m({ file: x, fileList: g }) {
      let S = (g || []).filter((G) => G.url).map((G) => G.url);
      a("update:value", S.join(",")), a("update:fileList", g);
    }
    return l({
      submitFile: () => {
        var x;
        (x = v.value) == null || x.submit();
      }
    }), (x, g) => (d(), O("div", _l, [
      U(s(ht), Te({
        ref: (S) => {
          v.value = S;
        },
        "file-list": o.value,
        "onUpdate:fileList": g[0] || (g[0] = (S) => o.value = S),
        onRemove: r,
        multiple: t.multipleUpload,
        "default-upload": t.autoUpload,
        "custom-request": w,
        onBeforeUpload: y,
        onChange: m
      }, s(n)), {
        default: C(() => {
          var S;
          return [
            K("span", kl, [
              ((S = s(n)) == null ? void 0 : S["list-type"]) === "image-card" ? (d(), O("span", xl, "+")) : W(x.$slots, "uploadBtn", {
                key: 1,
                loading: s(k)
              }, void 0, !0)
            ])
          ];
        }),
        _: 3
      }, 16, ["file-list", "multiple", "default-upload"])
    ]));
  }
}, wl = /* @__PURE__ */ oe(ml, [["__scopeId", "data-v-742f8af6"]]), Ne = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  BlueTitle: $e,
  Dialog: Ve,
  DialogMsg: At,
  Drawer: zt,
  Form: el,
  FormBtns: al,
  Info: sl,
  ListPage: Yt,
  LoginAgreement: pl,
  MarkDown: dl,
  TreeSync: hl,
  UploadFileList: wl
}, Symbol.toStringTag, { value: "Module" }));
const Nl = function(t, l = {}) {
  for (let a in Ne)
    a.startsWith("Xnsk") ? t.component(a, Ne[a]) : t.component("Xnsk" + a, Ne[a]);
}, Ol = H({ install: Nl }, Ne);
export {
  Ol as default
};
//# sourceMappingURL=xnsk-admin-ui.js.map
